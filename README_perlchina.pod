...
=item * 运行起步脚本。假如你挑选的是 C<data/origin/www.perl.org/%2Fget.html.new> 这个页面（其中的 %2F 其实就是 / ），那么把它作为参数传给起步脚本： C<perl start_trans.pl data/origin/www.perl.org/%2Fget.html.new> 。起步脚本会为你生成两个文件： C<data/origin/www.perl.org/%2Fget.html.old> ，这个是用来防止两个人同时翻译一个页面的，见上一条； C<data/trans/www.perl.org/%2Fget.html.old> ，你编辑这个文件，修改里面的英文就可以进行翻译了。
